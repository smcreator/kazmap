...
namespace OSMUpload
{
    public partial class OSMUploader : Form
    {
        ...
        private void BackgroundRun(object @params)
        {
            ...
            try
            {
                using (var dbf = new SqlConnection(@"Data Source=localhost\SQLEXPRESS;Initial Catalog=KazMap;Integrated Security=True;MultipleActiveResultSets=True"))
                {
                    dbf.Open();
                    if(files[0]!=null)
                    {
                        NotifyProgress("Uploading OSM file ...");
                        OSMReader.Read(files[0], ProgressNotify);
                    }
                    if(files[2]!=null)
                    {
